...
for env in envs:     
    ...
    env_vars_raw = {**env_vars, "DATABRICKS_DEPLOYMENT_NAME" : f"CUMULUS_PELASER_PRINTERNET_RAW_{env_for_deployment.upper()}"}
    raw_task_list=PeptoDatabricksTaskList(dag=dag, env=env,source_id=source_id,process="dps_raw",super_package="raw_pepto",package="dps_raw_pelaser_pepto",env_vars=env_vars_raw)
    raw_task_dict=raw_task_list.get_ops()
    
    env_vars_stdraw = {**env_vars, "DATABRICKS_DEPLOYMENT_NAME" : f"CUMULUS_PELASER_PRINTERNET_STDRAW_{env_for_deployment.upper()}"}
    ...
    env_vars_stdraw_old = {**env_vars, "DATABRICKS_DEPLOYMENT_NAME" : f"CUMULUS_PELASER_PRINTERNET_STDRAW_{env_for_deployment.upper()}_OLD"}
    ...
    env_vars_rs = {**env_vars, "DATABRICKS_DEPLOYMENT_NAME" : f"CUMULUS_PELASER_PRINTERNET_REDSHIFT_{env_for_deployment.upper()}"}
    rs_task_list=PeptoDatabricksTaskList(dag=dag, env=env,source_id=source_id,process="dps_redshiftloader",super_package="ink_stdraw_pepto",package="dps_redshiftloader",previous_task_ids=[stdraw_task_dict["run_op"].task_id],env_vars=env_vars_rs)
    ...
    env_vars_lake = {**env_vars, "DATABRICKS_DEPLOYMENT_NAME" : f"CUMULUS_PELASER_PRINTERNET_STDRAW_DELTA_LAKE_{env_for_deployment.upper()}"}
    ...
    env_vars_ocv = {**env_vars, "DATABRICKS_DEPLOYMENT_NAME" : f"CUMULUS_PELASER_PRINTERNET_OCV_{env_for_deployment.upper()}"}
    ocv_task_list=PeptoDatabricksTaskList(dag=dag, env=env,source_id=source_id,process="dps_ocv",super_package="ocv_pepto",package="dps_ocv_pepto", previous_task_ids=[stdraw_task_dict["run_op"].task_id],env_vars=env_vars_ocv,pool='ocv_pool')
