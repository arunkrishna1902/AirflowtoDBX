        ...
        if date_list != None:
            for i in date_list:
                ...
                dynamic_task = _get_dbt_operation_task(
                    ...
                    selector=f"""update_synthetic_privacy_object --profile ink --args "{{source_id: {source_id}, source_name: ink, start_date: {i[0]}, end_date: {i[1]}}}" """,
                    vars=dbt_vars,
                    env_vars=env_vars,
                    retries=retry_count,
                )
...
def populate_dag(dag, image, cumulus_vars, extra_vars=None, retry_on_failure=False):
    ...
    num_peink_batches = int(cumulus_vars["num_peink_batches"])
    num_wppgen1_ink_batches = int(cumulus_vars["num_wppgen1_ink_batches"])
    num_wppgen2_ink_batches = int(cumulus_vars["num_wppgen2_ink_batches"])
    num_gotham_ink_batches = int(cumulus_vars["num_gotham_ink_batches"])

    date_list_peink = batch_dates(
        list(
            date_range(start_date_peink,end_date_peink,num_peink_batches)
            ))
    date_list_wppgen1_ink = batch_dates(
        list(
            date_range(start_date_wppgen1_ink,end_date_wppgen1_ink,num_wppgen1_ink_batches)
            ))
    date_list_wppgen2_ink = batch_dates(
        list(
            date_range(start_date_wppgen2_ink,end_date_wppgen2_ink,num_wppgen2_ink_batches)
            ))
    date_list_gotham_ink = batch_dates(
        list(
            date_range(start_date_gotham_ink,end_date_gotham_ink,num_gotham_ink_batches)
            ))

    peink_task = DummyOperator(task_id="peink_start", dag=dag)
    wppgen1_ink_task = DummyOperator(task_id="wppgen1_ink_start", dag=dag)
    wppgen2_ink_task = DummyOperator(task_id="wppgen2_ink_start", dag=dag)
    gotham_ink_task = DummyOperator(task_id="gotham_ink_start", dag=dag)
    ...
    prev_task = peink_task
    for i in date_list_peink:
        batch_start = datetime.datetime.strptime(i[0], "%Y-%m-%d").strftime("%Y.%m.%d")
        batch_end = datetime.datetime.strptime(i[1], "%Y-%m-%d").strftime("%Y.%m.%d")
        
        add_spo_flag_peink = _get_dbt_operation_task(
            dag,
            f"add-spo-flag-peink-{batch_start}-{batch_end}",
            namespace,
            image,
            dbt_run_operation_cmd,
            selector=f"""update_synthetic_privacy_object --profile ink --args "{{source_id: PEINK, source_name: ink, start_date: {i[0]}, end_date: {i[1]}}}" """,
            vars=dbt_vars,
            env_vars=env_vars,
            retries=retry_count,
        )
        prev_task >> add_spo_flag_peink
        prev_task = add_spo_flag_peink
    prev_task >> wppgen1_ink_task
    prev_task = wppgen1_ink_task
    for i in date_list_wppgen1_ink:
        batch_start = datetime.datetime.strptime(i[0], "%Y-%m-%d").strftime("%Y.%m.%d")
        batch_end = datetime.datetime.strptime(i[1], "%Y-%m-%d").strftime("%Y.%m.%d")
        
        add_spo_flag_wppgen1_ink = _get_dbt_operation_task(
            dag,
            f"add-spo-flag-wppgen1-ink-{batch_start}-{batch_end}",
            namespace,
            image,
            dbt_run_operation_cmd,
            selector=f"""update_synthetic_privacy_object --profile ink --args "{{source_id: WPPGEN1, source_name: ink, start_date: {i[0]}, end_date: {i[1]}}}" """,
            vars=dbt_vars,
            env_vars=env_vars,
            retries=retry_count,
        )
        prev_task >> add_spo_flag_wppgen1_ink
        prev_task = add_spo_flag_wppgen1_ink

    prev_task >> wppgen2_ink_task
    prev_task = wppgen2_ink_task
    for i in date_list_wppgen2_ink:
        batch_start = datetime.datetime.strptime(i[0], "%Y-%m-%d").strftime("%Y.%m.%d")
        batch_end = datetime.datetime.strptime(i[1], "%Y-%m-%d").strftime("%Y.%m.%d")
        
        add_spo_flag_wppgen2_ink = _get_dbt_operation_task(
            dag,
            f"add-spo-flag-wppgen2-ink-{batch_start}-{batch_end}",
            namespace,
            image,
            dbt_run_operation_cmd,
            selector=f"""update_synthetic_privacy_object --profile ink --args "{{source_id: WPPGEN2_SCHEDULED, source_name: ink, start_date: {i[0]}, end_date: {i[1]}}}" """,
            vars=dbt_vars,
            env_vars=env_vars,
            retries=retry_count,
        )
        prev_task >> add_spo_flag_wppgen2_ink
        prev_task = add_spo_flag_wppgen2_ink

    prev_task >> gotham_ink_task
    prev_task = gotham_ink_task
    for i in date_list_gotham_ink:
        batch_start = datetime.datetime.strptime(i[0], "%Y-%m-%d").strftime("%Y.%m.%d")
        batch_end = datetime.datetime.strptime(i[1], "%Y-%m-%d").strftime("%Y.%m.%d")
        
        add_spo_flag_gotham_ink = _get_dbt_operation_task(
            dag,
            f"add-spo-flag-gotham-ink-{batch_start}-{batch_end}",
            namespace,
            image,
            dbt_run_operation_cmd,
            selector=f"""update_synthetic_privacy_object --profile ink --args "{{source_id: GOTHAM_LEDM, source_name: ink, start_date: {i[0]}, end_date: {i[1]}}}" """,
            vars=dbt_vars,
            env_vars=env_vars,
            retries=retry_count,
        )
        prev_task >> add_spo_flag_gotham_ink
        prev_task = add_spo_flag_gotham_ink
    test_ink_spo = _get_dbt_task(
        dag,
        "test-ink-spo",
        namespace,
        image,
        dbt_test_cmd,
        selector="--profile ink --models tag:synthetic_privacy_object_ink",
        vars=dbt_vars,
        env_vars=env_vars,
        retries=retry_count,
    )
