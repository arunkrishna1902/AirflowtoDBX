...
def populate_dag(dag, image, cumulus_vars, extra_vars=None, retry_on_failure=False):
    ...
    num_ocv_batches = (int(cumulus_vars["num_ocv_batches"]) * 2)

    date_list_ocv = batch_dates(
        list(
    ...
    for i in date_list_ocv:
        ...
        add_spo_strnec_flag_ocv = _get_dbt_operation_task(
            dag,
            f"add-spo-strnec-flag-{batch_start}-{batch_end}",
            namespace,
            image,
            dbt_run_operation_cmd,
            selector=f"""update_synthetic_privacy_object_ocv --profile ocv --args "{{table_name: ocv_history, start_date: {i[0]}, end_date: {i[1]}}}" """,
            vars=dbt_vars,
            env_vars=env_vars,
            retries=retry_count,
        )
        prev_task >> add_spo_strnec_flag_ocv
        prev_task = add_spo_strnec_flag_ocv

    test_ocv_spo = _get_dbt_task(
        dag,
        "test-ocv-spo",
        namespace,
        image,
        dbt_test_cmd,
        selector="--profile laser --models tag:synthetic_privacy_object_ocv",
        vars=dbt_vars,
        env_vars=env_vars,
        retries=retry_count,
    )

    test_ocv_strnec = _get_dbt_task(
        dag,
        "test-ocv-strnec",
        namespace,
        image,
        dbt_test_cmd,
        selector="--profile laser --models tag:eea_strnec_flag_ocv",
        vars=dbt_vars,
        env_vars=env_vars,
        retries=retry_count,
    )
    prev_task >> test_ocv_spo >> test_ocv_strnec
