...
def populate_dag(dag, image, cumulus_vars, extra_vars=None, retry_on_failure=False):
    ...
    for i in date_list_pmhit:
        ...
        add_strnec_flag_pmhit = _get_dbt_operation_task(
            dag,
            f"add-strnec-flag-{batch_start}-{batch_end}",
            namespace,
            image,
            dbt_run_operation_cmd,
            selector=f"""set_eea_strnec_flag_to_false --profile laser --args "{{source_name: laser, start_date: {i[0]}, end_date: {i[1]}}}" """,
            vars=dbt_vars,
            env_vars=env_vars,
            retries=retry_count,
        )
        prev_task >> add_strnec_flag_pmhit
        prev_task = add_strnec_flag_pmhit

    test_laser_strnec = _get_dbt_task(
        dag,
        "test-laser-strnec",
        namespace,
        image,
        dbt_test_cmd,
        selector="--profile laser --models tag:eea_strnec_flag_laser",
        vars=dbt_vars,
        env_vars=env_vars,
        retries=retry_count,
    )
