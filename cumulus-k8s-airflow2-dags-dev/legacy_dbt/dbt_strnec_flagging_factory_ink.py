...
def populate_dag(dag, image, cumulus_vars, extra_vars=None, retry_on_failure=False):
    ...
    for i in date_list_peink:
        ...
        add_strnec_flag_peink = _get_dbt_operation_task(
            dag,
            f"add-strnec-flag-{batch_start}-{batch_end}",
            namespace,
            image,
            dbt_run_operation_cmd,
            selector=f"""set_eea_strnec_flag_to_false --profile ink --args "{{source_name: ink, start_date: {i[0]}, end_date: {i[1]}}}" """,
            vars=dbt_vars,
            env_vars=env_vars,
            retries=retry_count,
        )
        prev_task >> add_strnec_flag_peink
        prev_task = add_strnec_flag_peink

    test_ink_strnec = _get_dbt_task(
        dag,
        "test-ink-strnec",
        namespace,
        image,
        dbt_test_cmd,
        selector="--profile ink --models tag:eea_strnec_flag_ink",
        vars=dbt_vars,
        env_vars=env_vars,
        retries=retry_count,
    )
