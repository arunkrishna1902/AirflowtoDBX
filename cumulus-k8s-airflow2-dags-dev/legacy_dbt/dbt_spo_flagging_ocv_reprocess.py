...
import dbt.dbt_spo_factory_flagging_ocv_reprocess as spo_factory
...
dag = DAG(
    dag_id="cumulus-dbt-spo-flagging-ocv-reprocess",
    default_args=args,
...
start_date_ocv = "2022-03-12"
end_date_ocv = "2022-03-13"


num_ocv_batches = 1

date_list_ocv = spo_factory.batch_dates(
    list(
        spo_factory.date_range(start_date_ocv,end_date_ocv,num_ocv_batches)
        ))
...
test_ocv_spo = spo_factory.build_taskgroup(dag=dag,
    group_id='test_ocv_spo',
    image=image,
    cumulus_vars=cumulus_vars,
    extra_vars=extra_vars,
    is_test=True,
    selector_stmt="--profile ocv --models tag:synthetic_privacy_object_ocv"
    )
prev_task >> test_ocv_spo
prev_task = test_ocv_spo

test_ocv_strnec = spo_factory.build_taskgroup(dag=dag,
    group_id='test_ocv_strnec',
    image=image,
    cumulus_vars=cumulus_vars,
    extra_vars=extra_vars,
    is_test=True,
    selector_stmt="--profile ocv --models tag:eea_strnec_flag_ocv"
    )
prev_task >> test_ocv_strnec
prev_task = test_ocv_strnec
