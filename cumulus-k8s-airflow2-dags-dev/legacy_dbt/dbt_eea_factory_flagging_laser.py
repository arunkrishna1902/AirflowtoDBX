...
def populate_dag(dag, image, cumulus_vars, extra_vars=None, retry_on_failure=False):
    ...
    #     image,
    #     dbt_run_cmd,
    #     selector="--profile laser --models tag:ocv_flagging",
    #     vars=dbt_vars,
    #     env_vars=env_vars,
    #     retries=retry_count,
    # )

    add_eea_flag_pelaser = _get_dbt_operation_task(
        dag,
        "add-eea-flag-pelaser",
        namespace,
        image,
        dbt_run_operation_cmd,
        selector=f"""rem_add_eea_flag_pelaser --profile laser --args "{{source_id: PELASER_DPP, start_date: {start_date_pelaser}, end_date: {end_date_pelaser}}}" """,
        vars=dbt_vars,
        env_vars=env_vars,
        retries=retry_count,
    )

    gotham_flag_no_allow_list_laser = _get_dbt_operation_task(
        dag,
        "gotham-flag-no-allow-list-laser",
        namespace,
        image,
        dbt_run_operation_cmd,
        selector=f"""rem_add_eea_flag_no_allow_list_laser --profile laser --args "{{source_id: GOTHAM_LEDM, start_date: {start_date_gotham}, end_date: {end_date_gotham}}}" """,
        vars=dbt_vars,
        env_vars=env_vars,
        retries=retry_count,
    )

    wppg2_flag_no_allow_list_laser = _get_dbt_operation_task(
        dag,
        "wppg2-flag-no-allow-list-laser",
        namespace,
        image,
        dbt_run_operation_cmd,
        selector=f"""rem_add_eea_flag_no_allow_list_laser --profile laser --args "{{source_id: WPPGEN2_SCHEDULED, start_date: {start_date_wppgen2}, end_date: {end_date_wppgen2}}}" """,
        vars=dbt_vars,
        env_vars=env_vars,
        retries=retry_count,
    )

    hppk_flag_no_allow_list_laser = _get_dbt_operation_task(
        dag,
        "hppk-flag-no-allow-list-laser",
        namespace,
        image,
        dbt_run_operation_cmd,
        selector=f"""rem_add_eea_flag_no_allow_list_laser --profile laser --args "{{source_id: HPPK_AMPV, start_date: {start_date_hppk}, end_date: {end_date_hppk}}}" """,
        vars=dbt_vars,
        env_vars=env_vars,
        retries=retry_count,
    )

    samsung_flag_no_allow_list_laser = _get_dbt_operation_task(
        dag,
        "samsung-flag-no-allow-list-laser",
        namespace,
        image,
        dbt_run_operation_cmd,
        selector=f"""rem_add_eea_flag_no_allow_list_laser --profile laser --args "{{source_id: SAMSUNG_AMPV, start_date: {start_date_samsung}, end_date: {end_date_samsung}}}" """,
        vars=dbt_vars,
        env_vars=env_vars,
        retries=retry_count,
    )

    set_null_to_false_pre_target_laser = _get_dbt_operation_task(
        dag,
        "set-null-to-false-pre-target-laser",
        namespace,
        image,
        dbt_run_operation_cmd,
        selector=f"""set_null_to_false_pre_target_laser --profile laser --args "{{start_date: {start_date_pre_target_laser}, end_date: {end_date_pre_target_laser}}}" """,
        vars=dbt_vars,
        env_vars=env_vars,
        retries=retry_count,
    )

    set_null_to_false_post_target_laser = _get_dbt_operation_task(
        dag,
        "set-null-to-false-post-target-laser",
        namespace,
        image,
        dbt_run_operation_cmd,
        selector=f"""set_null_to_false_post_target_laser --profile laser --args "{{start_date: {start_date_post_target_laser}, end_date: {end_date_post_target_laser}}}" """,
        vars=dbt_vars,
        env_vars=env_vars,
        retries=retry_count,
    )

    unload_flag_ids_laser = _get_dbt_operation_task(
        dag,
        "unload-flag-ids-laser",
        namespace,
        image,
        dbt_run_operation_cmd,
        selector="unload_flag_ids_laser --profile laser",
        vars=dbt_vars,
        env_vars=env_vars,
        retries=retry_count,
    )

    test_laser_flagging = _get_dbt_task(
        dag,
        "test-laser-flagging",
        namespace,
        image,
        dbt_test_cmd,
        selector="--profile laser --models tag:pelaser_flagging",
        vars=dbt_vars,
        env_vars=env_vars,
        retries=retry_count,
    )

    test_laser_set_false = _get_dbt_task(
        dag,
        "test-laser-set-false",
        namespace,
        image,
        dbt_test_cmd,
        selector="--profile laser --models tag:laser_set_false",
        vars=dbt_vars,
        env_vars=env_vars,
        retries=retry_count,
    )
    
    test_laser_count_compare_databricks = _get_databricks_task(
        dag,
        "test-laser-count-compare-databricks-trigger",
        namespace,
        dbx_image,
        dbx_command,
    ...
    (
        current_task
        >> [
            add_eea_flag_pelaser,
            gotham_flag_no_allow_list_laser,
            wppg2_flag_no_allow_list_laser,
            hppk_flag_no_allow_list_laser,
            samsung_flag_no_allow_list_laser,
        ]
        >> set_null_to_false_post_target_laser
        >> set_null_to_false_pre_target_laser
        >> [test_laser_flagging, test_laser_set_false]
        >> unload_flag_ids_laser
        >> test_laser_count_compare_databricks
    )
