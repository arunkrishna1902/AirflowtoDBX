...
#helper function to determine last day of any month
def get_last_day_of_month(any_day):
    next_month = any_day.replace(day=28) + datetime.timedelta(days=4)
    return next_month - datetime.timedelta(days=next_month.day)

#creates list of lists of dates
#ex: [[START_DATE, '2020-01-31'], ['2020-02-01', '2020-02-29'], ....['2020-12-01', END_DATE]]
def get_month_batches(begin,end):
    ...
    while True:
        ...
        if next_month > end:
            break
        result.append ([begin.strftime("%Y-%m-%d"),get_last_day_of_month(begin).strftime("%Y-%m-%d")])
        begin = next_month
...
def populate_dag(dag, image, cumulus_vars, extra_vars=None, retry_on_failure=False):
    ...
    date_list = get_month_batches(start_date_ocv,end_date_ocv)
    ...
    # for i in date_list:
    #     curr_month = datetime.datetime.strptime(i[0], "%Y-%m-%d").strftime('%b')
    #     curr_year = datetime.datetime.strptime(i[0], "%Y-%m-%d").strftime('%Y')
    ...
    ocv_flagging = _get_dbt_task(
        ...
        image,
        dbt_run_cmd,
        selector=f"""--profile laser --models tag:ocv_flagging""",
        vars=dbt_vars,
        env_vars=env_vars,
        retries=retry_count,
    )
    ...
    current_task >> ocv_flagging >> test_ocv_flagging
