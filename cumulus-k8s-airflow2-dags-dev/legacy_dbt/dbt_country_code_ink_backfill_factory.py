...
def populate_dag(dag, image, cumulus_vars, extra_vars=None, retry_on_failure=False):
    ...
    # process dates
    start_date_peink = cumulus_vars["process_dates"]["start_date_peink_country_code"]
    end_date_peink = cumulus_vars["process_dates"]["end_date_peink_country_code"]

    start_date_wppgen2_ink = cumulus_vars["process_dates"]["start_date_wppgen2_ink_country_code"]
    end_date_wppgen2_ink = cumulus_vars["process_dates"]["end_date_wppgen2_ink_country_code"]

    start_date_gotham_ink = cumulus_vars["process_dates"]["start_date_gotham_ink_country_code"]
    end_date_gotham_ink = cumulus_vars["process_dates"]["end_date_gotham_ink_country_code"]

    num_peink_batches = int(cumulus_vars["num_peink_batches"])
    num_wppgen2_batches = int(cumulus_vars["num_wppgen2_ink_batches"])
    num_gotham_batches = int(cumulus_vars["num_gotham_ink_batches"])
    ...
    date_list_wppgen2 = batch_dates(
        list(
            date_range(start_date_wppgen2_ink,end_date_wppgen2_ink,num_wppgen2_batches)
            ))
    date_list_gotham = batch_dates(
        list(
            date_range(start_date_gotham_ink,end_date_gotham_ink,num_gotham_batches)
            ))
    ...
    gen2_task = DummyOperator(task_id="gen2_start", dag=dag)
    ...
    # for i in date_list_wppgen2:
        ...
        # update_fw_country_code_wppgen2 = _get_dbt_operation_task(
            # dag,
            # f"wppgen2-{batch_start}-{batch_end}",
            # namespace,
            # image,
            # dbt_run_operation_cmd,
            # selector=f"""rem_fw_country_code --profile ink --args "{{source_id: WPPGEN2_SCHEDULED, start_date: {i[0]}, end_date: {i[1]}}}" """,
            # vars=dbt_vars,
            # env_vars=env_vars,
            # retries=retry_count,
        # )
        
        # prev_task >> update_fw_country_code_wppgen2
        # prev_task = update_fw_country_code_wppgen2
    ...
    # for i in date_list_gotham:
        ...
        # update_fw_country_code_gotham = _get_dbt_operation_task(
            # dag,
            # f"gotham-{batch_start}-{batch_end}",
            # namespace,
            # image,
            # dbt_run_operation_cmd,
            # selector=f"""rem_fw_country_code --profile ink --args "{{source_id: GOTHAM_LEDM, start_date: {i[0]}, end_date: {i[1]}}}" """,
            # vars=dbt_vars,
            # env_vars=env_vars,
            # retries=retry_count,
        # )
        
        # prev_task >> update_fw_country_code_gotham
        # prev_task = update_fw_country_code_gotham
    ...
    for i in date_list_peink:
        ...
        update_fw_country_code_peink = _get_dbt_operation_task(
            dag,
            f"peink-{batch_start}-{batch_end}",
            namespace,
            image,
            dbt_run_operation_cmd,
            selector=f"""rem_fw_country_code --profile ink --args "{{source_id: PEINK, start_date: {i[0]}, end_date: {i[1]}}}" """,
            vars=dbt_vars,
            env_vars=env_vars,
            retries=retry_count,
        )
        
        prev_task >> update_fw_country_code_peink
        prev_task = update_fw_country_code_peink
