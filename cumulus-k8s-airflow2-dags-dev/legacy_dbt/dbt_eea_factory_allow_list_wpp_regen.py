...
def populate_dag(dag, image, cumulus_vars, extra_vars=None, retry_on_failure=False):
    ...
    alter_wpp_allowlist = _get_dbt_operation_task(
        dag,
        "alter-wpp-allowlist",
        namespace,
        image,
        dbt_run_operation_cmd,
        selector=f"""alter_wpp_allow_list --profile ink""",
        vars=dbt_vars,
        env_vars=env_vars,
        retries=retry_count,
    )
    ...
    allowlist_wpp_ink_test = _get_dbt_task(
        ...
        image,
        dbt_test_cmd,
        selector="--profile ink --models tag:allowlist_wpp_ink --exclude tag:eea_ink",
        vars=dbt_vars,
        env_vars=env_vars,
        retries=retry_count,
    )
