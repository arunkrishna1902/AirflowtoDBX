...
source_id="BIZ_MODEL_PROD_STACK"
...
for env in envs:     
    ...
    env_vars_stdraw = {**env_vars, "DATABRICKS_DEPLOYMENT_NAME" : f"CUMULUS_BIZ_MODEL_PROD_STACK_{env_for_deployment.upper()}"}
    ...
    env_vars_lake = {**env_vars, "DATABRICKS_DEPLOYMENT_NAME" : f"CUMULUS_BIZ_MODEL_RSLOAD_{env_for_deployment.upper()}"}
    lake_task_list=PeptoDatabricksTaskList(dag=dag, env=env,source_id=source_id,process="redshiftloader", previous_task_ids=[stdraw_task_dict["run_op"].task_id],env_vars=env_vars_lake)
