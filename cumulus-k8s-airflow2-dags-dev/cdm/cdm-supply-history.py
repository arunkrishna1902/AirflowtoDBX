...
for env in envs:     
    ...
    env_vars_stdraw = {**env_vars, "DATABRICKS_DEPLOYMENT_NAME" : f"CUMULUS_SH_FLEET_PROD_STACK_{env_for_deployment.upper()}"}
    prod_fleet_task_list=PeptoDatabricksTaskList(dag=dag, env=env,source_id=source_id,process="prod_stack",env_vars=env_vars_stdraw)
    prod_fleet_task_dict=prod_fleet_task_list.get_ops()
    
    env_vars_stdraw = {**env_vars, "DATABRICKS_DEPLOYMENT_NAME" : f"CUMULUS_SH_FLEET_STAGE_STACK_{env_for_deployment.upper()}"}
    stg_fleet_task_list=PeptoDatabricksTaskList(dag=dag, env=env,source_id=source_id,process="stage_stack",env_vars=env_vars_stdraw)
    stg_fleet_task_dict=stg_task_list.get_ops()

    prod_env_vars = {**env_vars, "DATABRICKS_DEPLOYMENT_NAME" : f"CUMULUS_SH_DUNE_PROD_STACK_{env_for_deployment.upper()}"}
    prod_task_list=PeptoDatabricksTaskList(dag=dag, env=env,source_id=source_id,process="imagingdevice_prod_stack",env_vars=prod_env_vars)
    ...
    stg_task_list=PeptoDatabricksTaskList(dag=dag, env=env,source_id=source_id,process="imagingdevice_stage_stack",env_vars=stg_env_vars)
