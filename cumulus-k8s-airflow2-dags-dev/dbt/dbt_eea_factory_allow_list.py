...
service_account_name = Variable.get('cumulus-dbt_eea', deserialize_json=True).get('service_account_name')
...
def populate_dag(dag, image, cumulus_vars, extra_vars=None, retry_on_failure=False):
    ...
    #vars for databricks
    dbx_image = f"{Variable.get('ecr_repository')}{cumulus_vars['dbx_vars'].get('ecr_image')}"
    ...
    allowlist_peink = _get_dbt_task(
        dag,
        "allowlist-peink",
        namespace,
        image,
        dbt_run_cmd,
        selector="--profile ink --models tag:allowlist_peink",
        vars=dbt_vars,
        env_vars=env_vars,
        retries=retry_count,
    )

    allowlist_peink_test = _get_dbt_task(
        dag,
        "allowlist-peink-dbt-test",
        namespace,
        image,
        dbt_test_cmd,
        selector="--profile ink --models tag:allowlist_peink",
    ...
    allowlist_wpp_ink = _get_dbt_task(
        dag,
        "allowlist-wpp-ink",
        namespace,
        image,
        dbt_run_cmd,
        selector="--profile ink --models tag:allowlist_wpp_ink",
        vars=dbt_vars,
        env_vars=env_vars,
        retries=retry_count,
    )

    allowlist_wpp_ink_test = _get_dbt_task(
        dag,
        "allowlist-wpp-ink-dbt-test",
        namespace,
        image,
        dbt_test_cmd,
        selector="--profile ink --models tag:allowlist_wpp_ink",
    ...
    allowlist_pelaser = _get_dbt_task(
        dag,
        "allowlist-pelaser",
        namespace,
        image,
        dbt_run_cmd,
        selector="--profile laser --models tag:allowlist_pelaser",
        vars=dbt_vars,
        env_vars=env_vars,
        retries=retry_count,
    )

    allowlist_pelaser_test = _get_dbt_task(
        dag,
        "allowlist-pelaser-dbt-test",
        namespace,
        image,
        dbt_test_cmd,
        selector="--profile laser --models tag:allowlist_pelaser",
    ...
    allowlist_count_compare_databricks = _get_databricks_task(
        dag,
        "allowlist-count-compare-databricks-trigger",
        namespace,
        dbx_image,
        dbx_command,
    ...
    (
        current_task
        >> allowlist_peink
        >> allowlist_peink_test
    )
    (
        current_task
        >> allowlist_wpp_ink
        >> allowlist_wpp_ink_test
    )
    (
        current_task
        >> allowlist_pelaser
        >> allowlist_pelaser_test
    )
    (
        [allowlist_peink_test, allowlist_wpp_ink_test, allowlist_pelaser_test]
        >>allowlist_count_compare_databricks
    )
    return dag
